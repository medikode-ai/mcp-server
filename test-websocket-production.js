...
// Test the production MCP server using the custom domain
const ws = new WebSocket('wss://mcp.medikode.ai/mcp?x-api-key=mk_e434859e2f6b8684ffed0751d6c0ea3d866513327662c9d011e55af59d0f84ff');

let messageId = 1;

ws.on('open', function open() {
    console.log('WebSocket connection opened to mcp.medikode.ai');
...
ws.on('message', function message(data) {
    ...
    if (response.id === 1 && response.result) {
        console.log('\n--- Testing tools/list on production ---');
    ...
    if (response.id === 2 && response.result && response.result.tools) {
        console.log('\n--- Testing tools/call with process_chart on production ---');
    ...
    if (response.id === 4) {
        console.log('\n--- Production test completed, closing connection ---');
        setTimeout(() => {
            ws.close();
        }, 1000);
    }
